...
data = pk.load(open(get_path('heart_pred.sav'),'rb'))
scaler = pk.load(open(get_path('heart_scaler.sav'),'rb'))

st.title('توقع الشخص مصاب بمرض القلب ام لا')

image1 = get_path('sy.jpeg')
image2 = get_path('draw.png')

if os.path.exists(image1):
    st.image(image1, caption="نظام الذكاء الاصطناعي لتشخيص أمراض القلب", use_container_width=True)

st.sidebar.text('معلومات توضيحية')
if os.path.exists(image2):
    st.sidebar.image(image2, caption='توضيح تفصيلي ل ECG')
